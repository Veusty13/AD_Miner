...
def get_import_map(import_lines: List[str], root_path: Path = Path(".")) -> Dict[str, Dict[str, object]]:
...
class ControlInfo: 
    ...
    def get_control_dependencies(self) -> None: 
        import_lines: List[str] = [line for line in self.code_lines if line.startswith("from") or line.startswith("import")]
...
class ControlInfoGatherer:
    def __init__(self, output_folder: str, implemented_controls_map : Dict) -> None: 
